...
class EtfTrader:
    ...
    def calculate_spread(self, basket):
        ...
        try:
            ...
            const_prices = [const.wall_mid for const in sorted(constituents, key=lambda c: {s: i for i, s in enumerate(ETF_CONSTITUENT_SYMBOLS)}[c.name])]
    ...
    def get_basket_orders(self):
        ...
        for b_idx, basket in enumerate(self.baskets):
            ...
            if self.spreads[b_idx] > (BASKET_THRESHOLDS[b_idx] + informed_thr_adj) and basket.max_allowed_sell_volume > 0:
                ...
                if self.spreads[b_idx] is not None and self.spreads[b_idx] > informed_thr_adj and basket.initial_position > 0:
                ...
                elif self.spreads[b_idx] is not None and self.spreads[b_idx] < informed_thr_adj and basket.initial_position < 0:
