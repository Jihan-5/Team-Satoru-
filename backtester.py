...
STRATEGY v5 — v2 base (9,288 proven) + 3 targeted improvements:
  1. Multi-level take sweep: capture ALL mispriced levels, not just best
  2. PEPPER: prevent_adverse=False, take_width=-12, default_edge=-10, levels=5
     (faster max position build → more guaranteed drift PnL)
  3. ASH: levels 2→3 (slightly more passive depth)

v2 base — 7 data-driven improvements over v1
...
PARAMS = {
    ...
    ASH: {
        ...
        # - l2l1_coef 2.0→0.0: BIGGEST WIN. L2-L1 signal is noise on R2's volatile data.
        ...
        # - soft_position_limit 78→50: R2 is more volatile (|dev|>=15 happens ~1%/tick).
        #   Tighter position management prevents accumulating huge losing positions during drift.
        # - default_edge 7→6: tighter quotes capture more fills.
        # - kf_Q 1.0→3.0, kf_R 64→32: faster Kalman adaptation for volatile R2 data.
        # - take_width 1→2: slightly more selective takes (avoid marginal-edge trades).
        ...
        'soft_position_limit': 50,  # R2 TUNED (was 78): CRITICAL — prevents runaway positions
        'levels': 3,                # v5: slightly more passive depth (was 2)
    ...
    PEPPER: {
        ...
        'imb_coef': 0,
        'l2l1_coef': 0,
        'take_width': -12,          # v5: take asks ≤ fair+12 (was -8; buy anything near trend)
        'clear_width': 1,
        'prevent_adverse': False,   # v5: take ALL asks for PEPPER (drift guarantees profit)
        'adverse_volume': 15,
        'disregard_edge': 1,
        'join_edge': 0,
        'default_edge': -10,        # v5: bid at fair+10 (was -6; faster position build)
        'soft_position_limit': 80,
        'levels': 5,                # v5: deeper passive bids (was 4)
        'use_drift': True,
        'use_l2l1_signal': True,
...
class Trader:
    ...
    def bid(self):
        """
        Round 2 Market Access Fee bid — first-price sealed-bid auction.
        ...
        VALUATION MODEL:
          Extra 25% market flow → +25% on ASH only (PEPPER position-capped at 80).
          Our v15 baseline ASH PnL ≈ 1,700 (sandbox) × 9.23 (final scale) = 15,700.
          Tighter quotes (edge=5) convert extra flow to fills more efficiently.
          → MAF value estimate: 5,000–10,000 XIRECs (conservative midpoint = 7,500).
        
        BID DISTRIBUTION MODEL (640 participants, our prior):
          40% of teams bid 0       (didn't optimize MAF)
          30% bid 100–2,000        (low-effort optimizers)
          20% bid 2,000–7,000      (competitive optimizers)
          10% bid 7,000+           (aggressive overbidders)
          → estimated 90th percentile bid: ~6,000
          → estimated median: ~1,500
        
        DECISION:
          To target top 10%: bid ≈ 90th-percentile threshold = 6,000.
          To stay profitable: bid ≤ 0.9 × valuation = 6,750.
          Optimal bid = min(top_10pct_threshold, max_profitable_bid) = 6,000.
          
        RISK-AWARE ADJUSTMENT:
          Underbid risk: lose 5,000–10,000 opportunity (asymmetric loss)
          Overbid risk:  pay extra 1,000–2,000 if win
          → lean toward slightly higher than pure EV-optimal.
        """
        # --- Value estimation (in XIRECs) ---
        ASH_SANDBOX_PNL = 1700            # our proven ASH sandbox baseline
        FINAL_SCALE = 9.23                # sandbox → final simulation multiplier
        MAF_FLOW_BOOST = 0.25             # +25% extra market flow
        PEPPER_CAPPED = True              # PEPPER at position limit — no boost
        ...
        maf_value = ash_boost + pep_boost          # ≈ 3,925 pure ASH
        ...
        effective_value = maf_value * STRATEGY_MULTIPLIER   # ≈ 7,065
    ...
    def take_best_orders(
        ...
        # v5: Sweep ALL sell levels at or below fair - take_width (not just best)
        ...
        # Sweep ALL buy levels at or above fair + take_width
    ...
    def compute_fair(self, product, order_depth, trader_obj, state):
        ...
        ASH ('kalman_reversion' mode):
            ...
            Two-step predict/update cycle with adaptive gain K = P/(P+R):
              - High noise tick  → K small → filter stays stable, ignores spike
              - Trending tick    → K rises → filter tracks price faster
              - Settled tick     → K ≈ Q/(Q+R) ≈ 0.015 → smooth like EMA α=0.015
        ...
        PEPPER ('drift_plus_reversion' mode):
            Unchanged — EMA reversion + deterministic drift.
            drift_per_tick = 0.1 (R²=1.0000, confirmed 3/3 days).
        ...
        # v31: MM DETECTION — find MM bot's mid (volume 8-20 signature for ASH)
        MM_LO, MM_HI = 8, 20
        ...
        # ── ASH: Kalman filter replaces EMA reversion ──────────────────────
        if mode == 'kalman_reversion':
            ...
            R = p.get('kf_R', 64.0)  # measurement noise: microprice uncertainty ≈ (spread/2)²
            ...
            # Step 1 — Predict (no drift assumed for ASH; OU mean reverts)
            x_pred = x_prev
            P_pred = P_prev + Q

            # Step 2 — Update with microprice observation
            ...
            # L2-L1 mid signal (unchanged — independent predictor, corr 0.63-0.65)
        ...
        # ── PEPPER and fallback: original EMA reversion (unchanged) ────────
    ...
    def run(self, state: TradingState):
        ...
        for product in [ASH, PEPPER]:
            ...
            # Phase 2: Clear — v35 SMART CLEARING
            # For ASH: skip clear when position + signal align (favorable)
            # This lets us ride profitable moves instead of paying spread to close
            if not buy_only:
                skip_clear = False
                if product == ASH and od.buy_orders and od.sell_orders:
                    best_bid_c = max(od.buy_orders)
                    best_ask_c = min(od.sell_orders)
                    bv_c = abs(od.buy_orders[best_bid_c])
                    av_c = abs(od.sell_orders[best_ask_c])
                    tot_c = bv_c + av_c
                    if tot_c > 0:
                        imb_c = (bv_c - av_c) / tot_c
                        # LONG + up signal → HOLD (skip clear)
                        if pos > 5 and imb_c > 0.3:
                            skip_clear = True
                        # SHORT + down signal → HOLD (skip clear)
                        elif pos < -5 and imb_c < -0.3:
                            skip_clear = True
            ...
            # Sell into bids that are >= l1_mid + spike_threshold — genuine price spikes.
            if buy_only:
                ask_lvl = 0
            ...
            # When imb > 0.5: price likely UP (98% accurate) — widen ask, tighten bid
            # When imb < -0.5: price likely DOWN — widen bid, tighten ask
        ...
        # ── ASH DIAGNOSTICS — logs to help identify the PnL gap ──────────
